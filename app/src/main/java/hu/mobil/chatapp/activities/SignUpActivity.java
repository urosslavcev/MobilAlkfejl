...
public class SignUpActivity extends AppCompatActivity {
    ...
    private Boolean isValidSignUpDetails() {
        if (encodedImage == null) {
            showToast("Válaszd ki a profilképet!");
            return false;
        } else if (binding.inputName.getText().toString().trim().isEmpty()) {
            showToast("Írd be a neved!");
            return false;
        } else if (binding.inputEmail.getText().toString().trim().isEmpty()) {
            showToast("Írd be az email-ed!");
            return false;
        } else if (!Patterns.EMAIL_ADDRESS.matcher(binding.inputEmail.getText().toString()).matches()) {
            showToast("Nem valid email!");
            return false;
        } else if (binding.inputPassword.getText().toString().trim().isEmpty()) {
            showToast("Írd be a jelszavad!");
            return false;
        }else if (binding.inputConfirmPassword.getText().toString().trim().isEmpty()) {
            showToast("Jelszó nincs megerősítve!");
            return false;
        }else if(!binding.inputPassword.getText().toString().equals(binding.inputConfirmPassword.getText().toString())){
            showToast("Két jelszó nem egyezik meg!");
            return false;
        } else {
            return true;
        }
